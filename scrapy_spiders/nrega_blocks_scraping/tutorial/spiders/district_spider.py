...
class TestFullSpider(scrapy.Spider):
    ...
    'https://mnregaweb4.nic.in/netnrega/show_censuscode.aspx?page=s&lflag=eng&state_name=KARNATAKA&state_code=15&fin_year=2021-2022&source=national&Digest=z3lVd7ic0u/yHHasXbssXA'
    ]
    ...
    def parse_block(self, response):
        ...
        for row in rows:
            ...
            try:
                ...
                block_census_code = 'missing'
            except:
                block_census_code = "something else wrong"

            yield {'data_level': 'block',
                    'block_name': row.xpath('td//text()')[1].extract(),
                    'block_link': block_link,
                    'block_census_code': block_census_code
                    #'no_villages': row.xpath('td//text()')[3].extract(),
                    #'no_villages_with_code': row.xpath('td//text()')[4].extract()
                    }
